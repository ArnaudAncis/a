...
%       }
%       \visible<2>{
%         \draw[arrow] ($ (double) - (0,1.5) $) -- (double);
%       }
%       \visible<3>{
%         \draw[arrow] ($ (uint8.east) ! 0.5 ! (double.west) - (0,1.5) $) -- ($ (uint8.east) ! 0.5 ! (double.west)  $);
%       }
%       \visible<4>{
%         \draw[arrow] ($ (int16.north) + (0,1.5) $) -- (int16.north);
%       }
%     \end{tikzpicture}
%   \end{center}
%   \vskip5mm
...
%     \onslide<1>
%     \begin{center}
%       This is what you get from the WAV file \\
%       A \texttt{Stream<T>} is basically just a list of \texttt{T}s
%     \end{center}

%     \onslide<2>
%     \begin{center}
%       Our goal is to convert the raw data to a list of \texttt{double}s \\
%       \texttt{double}s will be much easier to work with later on
%     \end{center}

%     \onslide<3>
%     \begin{center}
%       If the WAV file contains 8 bit samples, we can convert these directly to \texttt{double}s
%     \end{center}

%     \onslide<4>
%     \begin{center}
%       If, however, the WAV file contains 16 bit samples, we will need an extra intermediary step:
%       pairs of \texttt{uint8} need to be combined into one \texttt{int16}
%     \end{center}

%     \onslide<5>
%     \begin{center}
%       Now that we have a stream of \texttt{double}s, we can separate the channels \\
%       We end up with one \texttt{Stream<double>} per channel
%     \end{center}

%     \onslide<6>
%     \begin{center}
%       Finally, we construct a continuous function based on the samples
%     \end{center}
%   \end{overprint}
% \end{frame}
